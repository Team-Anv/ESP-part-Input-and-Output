...
bool get_eve_PinCh1=0, get_eve_PinCh2=0, get_eve_PinCh3=0, get_eve_PinCh4=0, get_eve_PinCh5=0, get_eve_PinCh6=0, get_eve_PinCh7=0,get_eve_PinCh8=0;
String Ctr_Pin[20];
//************************
...
void ConfigIP_Address(int IPNo1, int IPNo2, int IPNo3, int IPNo4, int GwNo1, int GwNo2, int GwNo3,int GwNo4, int SubNo1, int SubNo2, int SubNo3, int SubNo4);
...
void setup() {
  ...
  setMode_PIN_D4(0);
  //ghi EEProm note
  if(int(EEPROM.read(AdEE_Status_PIN1_Start)) == 255 || int(EEPROM.read(AdEE_Status_PIN2_Start)) == 255 || int(EEPROM.read(AdEE_Status_PIN3_Start)) == 255)
  {
       Serial.println("va phan ghi DL Out or Input vao EEPROM!");
      WriteEEPROM_Acc(AdEE_Status_PIN1_Start,AdEE_Status_PIN1_End,"Tg"); // Tg/Sw/Pw
...
//  if (MDNS.begin("esp8266")) { //ESP.local/
...
// **************  USER INTERFACE********************
/***************PHAN CHUNG CHO 1 SO THAO TAC NHAY TRANG***************/
    ...
    String sPinNo, sU, sP,sControl;
      ...
      Serial.println(pass1);
       if((sU == user1 || sU == user2 || sU == user3 || sU == user4 || sU == user5) &&(sP == pass1 || sP == pass2 || sP == pass3 || sP == pass4 || sP == pass5))
       {
           ...
           else if(sPinNo == "7" && sControl !="")
           {
              Ctr_Pin[6] = sControl;
              get_eve_PinCh7 = true;
              Serial.print("gia tri cua thang Ctr_Pin7: ");
              Serial.println(Ctr_Pin[6]);
               request->send(200, "text/plain", "ok");
           }
           else if(sPinNo == "8" && sControl !="")
           {
              Ctr_Pin[7] = sControl;
              get_eve_PinCh8 = true;
              Serial.print("gia tri cua thang Ctr_Pin8: ");
              Serial.println(Ctr_Pin[7]);
               request->send(200, "text/plain", "ok");
           }
       }
       else {
   ...
   if (request->hasParam("Pin") && request->hasParam("SetMode") && request->hasParam("status")) {
    ...
    if(PinNumESP =="1")
    {
       if(SetModeESP == "INPUT" && sStatu != ""){
               ...
               request->send(200, "text/plain", "U can not config Pin 1 is INPUT, can U try again OUTPUT"); 
        }
...
void Config_IO()
{
  if(int(EEPROM.read(AdEE_Status_D3))==1)
  {
     if(int(EEPROM.read(AdEE_Status_Pin1))==1) 
       setMode_PIN_1(1);
     else setMode_PIN_1(0);
     
     if(int(EEPROM.read(AdEE_Status_Pin2))==1) 
       setMode_PIN_2(1);
     else setMode_PIN_2(0);
     
     if(int(EEPROM.read(AdEE_Status_Pin3))==1) 
       setMode_PIN_3(1);
     else setMode_PIN_3(0);

     if(int(EEPROM.read(AdEE_Status_Pin4))==1) 
       setMode_PIN_4(1);
     else setMode_PIN_4(0);

     if(int(EEPROM.read(AdEE_Status_Pin5))==1) 
       setMode_PIN_5(1);
     else setMode_PIN_5(0);

     if(int(EEPROM.read(AdEE_Status_Pin6))==1) 
       setMode_PIN_6(1);
     else setMode_PIN_6(0);

     if(int(EEPROM.read(AdEE_Status_Pin7))==1) 
       setMode_PIN_7(1);
     else setMode_PIN_7(0);

     if(int(EEPROM.read(AdEE_Status_Pin8))==1) 
       setMode_PIN_8(1);
     else setMode_PIN_8(0);
     Serial.println("theo cai dat!!!");
  }
  else ConfigPin_Default();
}
...
void ConfigIP_Address(int IPNo1, int IPNo2, int IPNo3, int IPNo4, int GwNo1, int GwNo2, int GwNo3,int GwNo4, int SubNo1, int SubNo2, int SubNo3, int SubNo4)
{
  IPAddress local_IP(IPNo1, IPNo2, IPNo3, IPNo4);
  // Set your Gateway IP address
  IPAddress gateway(GwNo1, GwNo2, GwNo3, GwNo4);
  IPAddress subnet(SubNo1, SubNo2, SubNo3, SubNo4);
